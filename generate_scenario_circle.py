# Python code to generate BlueSky scenarios of drones spawning within a certain radius from a centre point.
# Written by FreezingFalcon & Fazlur Rahman
...
def get_tangent_angle(drone_position, airport_center, radius):
    ...
    if(d > radius):
        theta = atan2(-dy, -dx)
        beta = asin(radius/d)
        return degrees(theta - beta) + 180, degrees(theta + beta) - 180
...
def df_to_scenario(df, file_id):
    for idx, row in df.iterrows():
        text = f"{'00:00:00.00>'} CRE XXX{idx:04d} M600 {row.lat} {row.lon} {row.heading} {row.alt} {row.spd}"
